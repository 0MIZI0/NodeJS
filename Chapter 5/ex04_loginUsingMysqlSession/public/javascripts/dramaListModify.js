...
function chooseOnlyOneCheckBox(selectedCheckBox, id){
    ...
    if(selectedCheckBox.checked){     // 체크박스를 체크한 경우
        ...
        document.querySelector('form').setAttribute('action', `/drama/update/${id}`)        // form 태그의 action 속성을 update 요청으로 수정
        document.getElementById('deleteBtn').setAttribute('href', `/drama/delete/${id}`)    // deleteBtn href 속성 수정
